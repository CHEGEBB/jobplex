...
class UserService {
  async getUserById(id: number): Promise<IUserResponse> {
    const result = await pool.query(
      `SELECT id, email, first_name, last_name, role, company, position, 
  ...
  async updateUser(id: number, userData: Partial<IUser>): Promise<IUserResponse> {
    ...
    const result = await pool.query(
      `UPDATE users SET ${updates.join(', ')} 
       ...
       RETURNING id, email, first_name, last_name, role, company, 
                 position, location, bio, phone, created_at, updated_at`,
      values
    );
  ...
  async getAllUsers(page: number = 1, limit: number = 10): Promise<{ users: IUserResponse[], total: number }> {
