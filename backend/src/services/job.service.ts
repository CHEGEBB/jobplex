...
class JobService {
  async createJob(jobData: IJob, skills: number[] = []): Promise<IJobWithSkills> {
    ...
    try {
      ...
      if (skills.length > 0) {
        for (const skillId of skills) {
          await client.query(
            'INSERT INTO job_skills (job_id, skill_id, importance, created_at, updated_at) VALUES ($1, $2, $3, NOW(), NOW())',
            [job.id, skillId, 1] // Default importance
          );
        }
      }
  ...
  async getJobById(id: number): Promise<IJobWithSkills> {
    // Get job details
  ...
  async getAllJobs(
    ...
    let skillsByJobId: {[key: number]: string[]} = {};
    ...
    if (jobIds.length > 0) {
      ...
      `;
      const skillsResult = await pool.query(skillsQuery, [jobIds]);
  ...
  async updateJob(id: number, jobData: Partial<IJob>, skills?: number[]): Promise<IJobWithSkills> {
    const client = await pool.connect();
      ...
      if (skills !== undefined) {
        ...
        if (skills.length > 0) {
          for (const skillId of skills) {
            await client.query(
              'INSERT INTO job_skills (job_id, skill_id, importance, created_at, updated_at) VALUES ($1, $2, $3, NOW(), NOW())',
              [id, skillId, 1] // Default importance
            );
          }
        }
      }
