...
exports.getJobMatches = exports.applyForJob = exports.deleteJob = exports.updateJobStatus = exports.updateJob = exports.createJob = exports.getJobById = exports.getEmployerJobs = exports.getAllJobs = void 0;
const db_config_1 = __importDefault(require("../config/db.config"));
// Get all jobs (with filters)
const getAllJobs = async (req, res) => {
    try {
        const { title, location, jobType, skill, page = '1', limit = '10' } = req.query;
        let query = `
    ...
    `;
        ...
        const [jobsResult, countResult] = await Promise.all([
            db_config_1.default.query(query, queryParams),
            db_config_1.default.query(countQuery, countParams)
        ]);
...
const getEmployerJobs = async (req, res) => {
    ...
    `;
        const jobsResult = await db_config_1.default.query(jobsQuery, [userId]);
...
const getJobById = async (req, res) => {
    ...
    `;
        const result = await db_config_1.default.query(jobQuery, [id]);
        ...
        if (userId && userId !== result.rows[0].employer_id) {
            await db_config_1.default.query(`INSERT INTO job_views (job_id, user_id) 
         VALUES ($1, $2) 
         ON CONFLICT (job_id, user_id) DO NOTHING`, [id, userId]);
        }
...
const createJob = async (req, res) => {
    ...
    const userId = req.user?.id;
    const { title, description, location, salary_range, job_type, status = 'draft', department, work_mode, experience_level, education_level, requirements, benefits, deadline_date: rawDeadlineDate, min_salary, max_salary, salary_visible = true, skills, screening_questions, internal = true, career_site = true, linkedin = false, indeed = false, team_access = 'all', scheduled_date } = req.body;
    ...
    try {
      ...
      RETURNING *`, [
        ...
        if (skills && skills.length > 0) {
            const skillValues = skills.map((skill) => `(${job.id}, '${skill.skillName.replace(/'/g, "''")}', '${skill.importance || 'required'}')`).join(', ');
            await client.query(`
      ...
      `);
        ...
        if (screening_questions && screening_questions.length > 0) {
            for (const question of screening_questions) {
                await client.query(`INSERT INTO screening_questions (
      ...
      GROUP BY j.id
    `;
        const completeJob = await db_config_1.default.query(completeJobQuery, [job.id]);
...
const updateJob = async (req, res) => {
    const { id } = req.params;
    const userId = req.user?.id;
    const { title, description, location, salary_range, job_type, status, department, work_mode, experience_level, education_level, requirements, benefits, deadline_date, min_salary, max_salary, salary_visible, skills, screening_questions, internal, career_site, linkedin, indeed, team_access, scheduled_date } = req.body;
    ...
    try {
        ...
        if (skills) {
            ...
            // Insert new skills
            if (skills.length > 0) {
                const skillValues = skills.map((skill) => `(${id}, '${skill.skillName.replace(/'/g, "''")}', '${skill.importance || 'required'}')`).join(', ');
                await client.query(`
          INSERT INTO job_skills (job_id, skill_name, importance)
          VALUES ${skillValues}
        `);
      ...
      GROUP BY j.id
    `;
        const completeJob = await db_config_1.default.query(completeJobQuery, [id]);
...
const updateJobStatus = async (req, res) => {
    ...
    try {
        // Check if job exists and belongs to the user
        const jobCheck = await db_config_1.default.query('SELECT * FROM jobs WHERE id = $1', [id]);
        if (jobCheck.rows.length === 0) {
            return res.status(404).json({ message: 'Job not found' });
        }
        if (jobCheck.rows[0].employer_id !== userId) {
        ...
        // Update status
        const updateResult = await db_config_1.default.query(`UPDATE jobs SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 AND employer_id = $3 RETURNING *`, [status, id, userId]);
        // Get the updated job with skills
        const completeJobQuery = `
      SELECT j.*, 
      COALESCE(
...
const deleteJob = async (req, res) => {
    ...
    try {
        ...
        await db_config_1.default.query('DELETE FROM jobs WHERE id = $1 AND employer_id = $2', [id, userId]);
...
const applyForJob = async (req, res) => {
    ...
    try {
        // Check if job exists
        const jobCheck = await db_config_1.default.query('SELECT * FROM jobs WHERE id = $1', [jobId]);
        if (jobCheck.rows.length === 0) {
            return res.status(404).json({ message: 'Job not found' });
        }
        // Check if user has already applied
        const applicationCheck = await db_config_1.default.query('SELECT * FROM applications WHERE job_id = $1 AND user_id = $2', [jobId, userId]);
        ...
        // Create application
        const applicationResult = await db_config_1.default.query('INSERT INTO applications (job_id, user_id, status, match_score) VALUES ($1, $2, $3, $4) RETURNING *', [jobId, userId, 'pending', matchScore]);
        res.status(201).json({
...
const getJobMatches = async (req, res) => {
    ...
    try {
        // Get user skills
        const userSkills = await db_config_1.default.query('SELECT name FROM skills WHERE user_id = $1', [userId]);
    ...
    `;
        const matches = await db_config_1.default.query(matchesQuery);
        res.json({
...
async function calculateMatchScore(userId, jobId) {
    try {
        // Get user skills
        const userSkills = await db_config_1.default.query('SELECT name, proficiency FROM skills WHERE user_id = $1', [userId]);
        // Get job skills
        const jobSkills = await db_config_1.default.query('SELECT skill_name, importance FROM job_skills WHERE job_id = $1', [jobId]);
